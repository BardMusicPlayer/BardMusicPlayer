...
namespace BardMusicPlayer.Quotidian.UtcMilliTime
{
    public sealed class Clock : ITime
    {
        ...
        private static long GetDeviceTime() => DateTime.UtcNow.Ticks / Constants.dotnet_ticks_per_millisecond -
        ...
        private static void PartB(IAsyncResult ar)
        {
            try
            {
                ...
                ntpCall.socket.BeginSend(ntpCall.buffer, 0, Constants.bytes_per_buffer, 0, new AsyncCallback(PartC),
                    null);
                ntpCall.methodsCompleted += 1;
            }
            catch (Exception)
        ...
        private static void PartC(IAsyncResult ar)
        {
            try
            {
                ...
                ntpCall.socket.BeginReceive(ntpCall.buffer, 0, Constants.bytes_per_buffer, 0, new AsyncCallback(PartD),
                    null);
                ntpCall.methodsCompleted += 1;
            }
            catch (Exception)
