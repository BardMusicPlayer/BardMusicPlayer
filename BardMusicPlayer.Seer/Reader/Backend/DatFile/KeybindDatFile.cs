...
internal class KeybindDatFile : IDisposable
{
    ...
    internal bool Load()
    {
        if (string.IsNullOrEmpty(_filePath)) throw new Exception("No path to KEYBIND.DAT file provided.");

        if (!File.Exists(_filePath)) throw new Exception("Missing KEYBIND.DAT file.");
        ...
        if (sourceSize - fileSize != 32)
        {
            reader.Dispose();
            memStream.Dispose();
            throw new Exception("Invalid KEYBIND.DAT size.");
        }
        ...
        {
            throw new Exception("Invalid HOTBAR.DAT format: " + ex.Message);
        }
        finally
        {
            reader.Dispose();
